...
export default function TripsPage() {
  ...
  useEffect(() => {
    fetch('https://airbnb-web-app-5i40.onrender.com/users/')
      ...
      .then(users => {
        ...
        if (guest) {
          return fetch(`https://airbnb-web-app-5i40.onrender.com/bookings/guest/${guest.id}`);
        }
      ...
      .then(async data => {
        const enriched = await Promise.all(data.map(async (b: any) => {
          const lRes = await fetch(`https://airbnb-web-app-5i40.onrender.com/listings/${b.listing_id}`);
  ...
  const submitReview = async () => {
    try {
      ...
      const res = await fetch(`https://airbnb-web-app-5i40.onrender.com/reviews/?guest_id=${guestId}`, {
        method: 'POST',
