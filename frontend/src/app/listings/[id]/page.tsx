...
async function getListing(id: string) {
  try {
    const res = await fetch(`https://airbnb-web-app-5i40.onrender.com/listings/${id}`, { next: { revalidate: 0 } });
...
async function getHost(hostId: string) {
  try {
    const res = await fetch(`https://airbnb-web-app-5i40.onrender.com/users/`, { next: { revalidate: 0 } });
