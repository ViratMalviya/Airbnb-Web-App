...
export default function HostDashboard() {
    ...
    fetch('http://127.0.0.1:8000/users/')
      ...
      .then(users => {
        ...
        if (host) {
          setHostId(host.id);
          return Promise.all([
            fetch(`http://127.0.0.1:8000/listings/host/${host.id}`).then(r => r.json()),
            fetch(`http://127.0.0.1:8000/bookings/host/${host.id}`).then(r => r.json())
          ]);
        }
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const url = editingId 
        ? `http://127.0.0.1:8000/listings/${editingId}`
        : `http://127.0.0.1:8000/listings/?host_id=${hostId}`;
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/listings/${id}`, { method: 'DELETE' });
      if(res.ok) {
